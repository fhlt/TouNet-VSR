...
class DatasetFromFolder(data.Dataset):
    def __init__(self, image_dir,nFrames, upscale_factor, data_augmentation, file_list, other_dataset, patch_size, future_frame, transform=None):
        ...
        self.video_list = os.listdir(image_dir)
        random.shuffle(self.video_list)
        self.frame_index = list(range(100))
    
    def load_image_future(self, idx_video, idx_frame):
        tt = int(self.nFrames / 2)
        filepath = os.path.join(self.image_dir,self.video_list[idx_video])
        target = Image.open(os.path.join(filepath, 'hr', 'hr' + str(idx_frame) + '.bmp')).convert('RGB')
        src = Image.open(os.path.join(filepath, 'lr_x4_BI','lr'+ str(idx_frame) + '.bmp')).convert('RGB')
        neigbor = []
        if self.nFrames % 2 == 0:
        ...
        for i in seq:
            index = idx_frame + i
            filepath1 = os.path.join(filepath, "lr_x4_BI", 'lr' + str(index) + '.bmp')
            if os.path.exists(filepath1):
                temp = Image.open(filepath1).convert('RGB')
            else:
                temp = src
            neigbor.append(temp)
        return target, src, neigbor

    def __getitem__(self, idx):
        idx_video = idx // 100
        if idx % 100 == 0:
            random.shuffle(self.frame_index)
        idx_frame = self.frame_index[idx % 100]
        target, input, neigbor = self.load_image_future(idx_video, idx_frame +1)
        '''
        ...
        flow = []
        # flow
        un_flow = get_flow(input, input)
        for i in range(len(neigbor)):
            tmp_flow = get_flow(input, neigbor[i])
            if np.std(tmp_flow) > 10:
                tmm_flow = un_flow
                neigbor[i] = input
...
class DatasetFromFolderTest(data.Dataset):
    def __init__(self, image_dir, video_name, nFrames, upscale_factor, file_list, other_dataset, future_frame, transform=None):
        ...
        self.other_dataset = other_dataset
        self.future_frame = future_frame
        if not os.path.exists(os.path.join('./flow', self.video_name)):
            os.makedirs(os.path.join('./flow', self.video_name))
    
    def load_image_future(self, idx_frame):
        tt = int(self.nFrames / 2)
        filepath = self.image_dir
        #src = cv2.imread(os.path.join(filepath, 'lr' + str(idx_frame) + ',bmp'))
        src = Image.open(os.path.join(filepath, 'lr'+ str(idx_frame) +'.bmp')).convert('RGB')
        neigbor = []
        if self.nFrames % 2 == 0:
            seq = [x for x in range(-tt, tt) if x != 0]
        else:
        ...
        for i in seq:
            index = idx_frame + i
            filepath1 = os.path.join(filepath, 'lr' + str(index) + '.bmp')
            if os.path.exists(filepath1):
                temp = Image.open(filepath1)
            else:
                temp = src
                index = idx_frame
            neigbor_idx.append(index)
            neigbor.append(temp)
        return src, neigbor,  neigbor_idx

    def __getitem__(self, index):
        input, neigbor, neigbor_idx = self.load_image_future(index+1)
        '''
        if self.future_frame:
            target, input, neigbor = load_img_future(self.image_filenames[index], self.nFrames, self.upscale_factor, self.other_dataset)
        else:
        ...
        # flow
        for i in range(len(neigbor)):
            flow_path = os.path.join('./flow', self.video_name,\
                    str(index + 1)+'_'+str(neigbor_idx[i])+'.npy')
            if os.path.exists(flow_path):
                a = np.load(flow_path)
                # if flow between neigbor and input is 0,
                # copy input to neigbor
                if np.mean(a) == 0:
                    neigbor[i] = input
                flow.append(np.load(flow_path))
            else:
                tmp_flow = get_flow(input, neigbor[i])
                if np.std(tmp_flow) > 10:
                    tmp_flow = get_flow(input, input)
                    neigbor[i] = input
                np.save(flow_path, tmp_flow)
                flow.append(tmp_flow)
        ...
        return input, neigbor, flow, bicubic
