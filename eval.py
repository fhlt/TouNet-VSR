...
parser.add_argument('--model',default='weights/4x_ubuntu-W580-G20TouNetF7_epoch_6.pth', help='model')
...
test_set = get_training_set(opt.data_dir, opt.nFrames, opt.upscale_factor, False, opt.file_list, opt.other_dataset, 0, opt.future_frame)
...
def eval():
    ...
    for batch in testing_data_loader:
        ...
        psnr_predicted = measure.compare_psnr(prediction, target, 255)
        avg_psnr_predicted += psnr_predicted
    
        print_test_log("===> Processing: %s || Timer: %.4f sec || psnr: %.6f || avg_psnr: %.6f " % (str(count), (t1 - t0), psnr_predicted, avg_psnr_predicted/count))
        count+=1
    print("PSNR_predicted=", avg_psnr_predicted/count)
    print_test_log("PSNR_predicted = %.6f" % (avg_psnr_predicted/count))
