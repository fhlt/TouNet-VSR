...
parser.add_argument('--model',
        default='weights/4x_ubuntu-W580-G20TouNetF7_epoch_20.pth', help='sr pretrained base model')
#        default='weights/4x_ubuntu-W580-G20RBPNF7_epoch_6.pth', help='sr pretrained base model')
...
test_set = get_test_set(os.path.join(opt.data_dir, "lr_x4"), opt.nFrames, opt.upscale_factor, opt.file_list, opt.other_dataset, opt.future_frame)
...
def eval(data_loader, video_name):
    ...
    for batch in data_loader:
        ...
        save_img(prediction.cpu().data, count, video_name, True)
        ...
        '''
        print_log("===> Processing: %s || Timer: %.4f sec." %
                (str(count), (t1 - t0)))
...
def save_img(img, img_name, video_name, pred_flag):
    ...
    '''
    save_fn = os.path.join(save_dir, "sr_" + str(img_name).rjust(3, '0') + '.bmp')
...
##Eval Start!!!!
for video_name in sorted(os.listdir(opt.data_dir))[:25]:
    print(video_name)
    test_set = get_test_set(os.path.join(opt.data_dir, video_name, 'lr_x4'), video_name, opt.nFrames, opt.upscale_factor, opt.file_list, opt.other_dataset, opt.future_frame)
    testing_data_loader = DataLoader(dataset=test_set, num_workers=opt.threads, batch_size=opt.testBatchSize, shuffle=False)
    eval(testing_data_loader, video_name)
